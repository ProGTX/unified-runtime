...
///        channel types.
/// /param return_hip_format will be set to the equivalent cuda
...
UR_APIEXPORT ur_result_t UR_APICALL urUSMPitchedAllocExp(
  ...
  try {
    ...
    UR_CHECK_ERROR(hipMemAllocPitch((hipDeviceptr_t *)ppMem, pResultPitch,
...
urBindlessImagesUnsampledImageHandleDestroyExp(
  ...
  UR_CHECK_ERROR(hipDestroySurfaceObject((hipSurfaceObject_t)hImage));
  return UR_RESULT_SUCCESS;
}
...
urBindlessImagesSampledImageHandleDestroyExp(
  ...
  UR_CHECK_ERROR(hipTexObjectDestroy((hipTextureObject_t)hImage));
  return UR_RESULT_SUCCESS;
}
...
UR_APIEXPORT ur_result_t UR_APICALL urBindlessImagesImageFreeExp(
  ...
  ScopedDevice Active(hDevice);
  try {
    UR_CHECK_ERROR(hipArrayDestroy((hipArray_t)hImageMem));
  } catch (ur_result_t Err) {
    return Err;
  } catch (...) {
    return UR_RESULT_ERROR_UNKNOWN;
...
UR_APIEXPORT ur_result_t UR_APICALL urBindlessImagesUnsampledImageCreateExp(
  ...
  try {
    ...
    image_res_desc.res.array.array = (hipArray_t)hImageMem;
...
UR_APIEXPORT ur_result_t UR_APICALL urBindlessImagesSampledImageCreateExp(
  ...
  try {
    ...
    // a hipArray_t or hipMipmappedArray_t. If this function returns
    // successfully, we check whether hImageMem is device memory (even managed
    // memory isn't considered shared).
    hipError_t Err =
        hipPointerGetAttribute(&mem_type, HIP_POINTER_ATTRIBUTE_MEMORY_TYPE,
                               (hipDeviceptr_t)hImageMem);
    if (Err != HIP_SUCCESS) {
      // We have a hipArray_t
      if (pImageDesc->numMipLevel == 1) {
        image_res_desc.resType = HIP_RESOURCE_TYPE_ARRAY;
        image_res_desc.res.array.hArray = (hipArray_t)hImageMem;
      }
      // We have a hipMipmappedArray_t
        ...
        image_res_desc.res.mipmap.hMipmappedArray =
            (hipMipmappedArray_t)hImageMem;
      }
    } else if (mem_type == hipMemoryTypeDevice) {
      // We have a USM pointer
      if (pImageDesc->type == UR_MEM_TYPE_IMAGE1D) {
        image_res_desc.resType = HIP_RESOURCE_TYPE_LINEAR;
        image_res_desc.res.linear.devPtr = (hipDeviceptr_t)hImageMem;
        ...
        image_res_desc.res.pitch2D.devPtr = (hipDeviceptr_t)hImageMem;
  ...
  try {
    ...
    if (imageCopyFlags == UR_EXP_IMAGE_COPY_FLAG_HOST_TO_DEVICE) {
      if (pDstImageDesc->type == UR_MEM_TYPE_IMAGE1D) {
        ...
        // somthing different from HIP_SUCCESS then we know that pDst memory
        // type is a CuArray. Otherwise, it's hipMemoryTypeDevice.
        bool isHipArray =
            hipPointerGetAttribute(&memType, HIP_POINTER_ATTRIBUTE_MEMORY_TYPE,
                                   (hipDeviceptr_t)pDst) != HIP_SUCCESS;
        ...
        if (pDstImageDesc->rowPitch == 0) {
          ...
          cpy_desc.dstArray = (hipArray_t)pDst;
        } else {
          // Pitched memory
          cpy_desc.dstMemoryType = hipMemoryTypeDevice;
          cpy_desc.dstDevice = (hipDeviceptr_t)pDst;
...
#if TODO
        UR_CHECK_ERROR(hipMemcpy3DAsync(&cpy_desc, Stream));
#endif
      ...
      if (pSrcImageDesc->type == UR_MEM_TYPE_IMAGE1D) {
        ...
        // somthing different from HIP_SUCCESS then we know that pSrc memory
        // type is a CuArray. Otherwise, it's hipMemoryTypeDevice.
        bool isHipArray =
            hipPointerGetAttribute(&memType, HIP_POINTER_ATTRIBUTE_MEMORY_TYPE,
                                   (hipDeviceptr_t)pSrc) != HIP_SUCCESS;
        ...
        if (isHipArray) {
          ...
          const char *SrcWithOffset =
              ...
              (pCopyRegion->srcOffset.x * PixelSizeBytes);
          UR_CHECK_ERROR(hipMemcpyDtoHAsync(DstWithOffset,
                                            (hipDeviceptr_t)SrcWithOffset,
                                            CopyExtentBytes, Stream));
        } else {
          // This should be unreachable.
          return UR_RESULT_ERROR_INVALID_VALUE;
...
UR_APIEXPORT ur_result_t UR_APICALL urBindlessImagesImageGetInfoExp(
  ...
  hipArray_t hhipArray_t;
  hipError_t Err = hipMipmappedArrayGetLevel(
      &hhipArray_t, reinterpret_cast<hipMipmappedArray_t>(hImageMem), 0);

  // If cuMipmappedArrayGetLevel failed, hImageMem is already hipArray_t.
  if (Err != HIP_SUCCESS) {
    hhipArray_t = reinterpret_cast<hipArray_t>(hImageMem);
  }
...
UR_APIEXPORT ur_result_t UR_APICALL urBindlessImagesMipmapGetLevelExp(
  ...
  try {
    ...
    UR_CHECK_ERROR(hipMipmappedArrayGetLevel(
        &ImageArray, (hipMipmappedArray_t)hImageMem, mipmapLevel));
    *phImageMem = (ur_exp_image_mem_native_handle_t)ImageArray;
  } catch (ur_result_t Err) {
    return Err;
  } catch (...) {
...
UR_APIEXPORT ur_result_t UR_APICALL urBindlessImagesMipmapFreeExp(
  ...
  ScopedDevice Active(hDevice);
  try {
    UR_CHECK_ERROR(hipMipmappedArrayDestroy((hipMipmappedArray_t)hMem));
  } catch (ur_result_t Err) {
    return Err;
  } catch (...) {
    return UR_RESULT_ERROR_UNKNOWN;
...
UR_APIEXPORT ur_result_t UR_APICALL urBindlessImagesMapExternalArrayExp(
  ...
  try {
    ...
    // External memory is mapped to a hipMipmappedArray_t
    // If desired, a hipArray_t is retrieved from the mipmaps 0th level
    hipMipmappedArray_t memMipMap;
    UR_CHECK_ERROR(hipExternalMemoryGetMappedMipmappedArray(
        &memMipMap, (hipExternalMemory_t)hExternalMem, &mipmapDesc));

    if (pImageDesc->numMipLevel > 1) {
      *phImageMem = (ur_exp_image_mem_native_handle_t)memMipMap;
    } else {
      hipArray_t memArray;
      UR_CHECK_ERROR(hipMipmappedArrayGetLevel(&memArray, memMipMap, 0));
...
UR_APIEXPORT ur_result_t UR_APICALL urBindlessImagesMapExternalLinearMemoryExp(
  ...
  try {
    ...
    UR_CHECK_ERROR(hipExternalMemoryGetMappedBuffer(
        &retMem, (hipExternalMemory_t)hExternalMem, &BufferDesc));
...
UR_APIEXPORT ur_result_t UR_APICALL urBindlessImagesReleaseExternalMemoryExp(
  ...
  try {
    ScopedDevice Active(hDevice);
    UR_CHECK_ERROR(hipDestroyExternalMemory((hipExternalMemory_t)hExternalMem));
  } catch (ur_result_t Err) {
    return Err;
  } catch (...) {
    return UR_RESULT_ERROR_UNKNOWN;
...
UR_APIEXPORT ur_result_t UR_APICALL urBindlessImagesReleaseExternalSemaphoreExp(
  ...
  try {
    ...
    UR_CHECK_ERROR(hipDestroyExternalSemaphore(
        (hipExternalSemaphore_t)hExternalSemaphore));
  } catch (ur_result_t Err) {
    return Err;
  } catch (...) {
    return UR_RESULT_ERROR_UNKNOWN;
...
UR_APIEXPORT ur_result_t UR_APICALL urBindlessImagesWaitExternalSemaphoreExp(
  ...
  try {
    ...
    UR_CHECK_ERROR(hipWaitExternalSemaphoresAsync(
        (hipExternalSemaphore_t *)&hSemaphore, &SemWaitParams,
        1 /* numExtSems */, Stream));
...
UR_APIEXPORT ur_result_t UR_APICALL urBindlessImagesSignalExternalSemaphoreExp(
  ...
  try {
    ...
    UR_CHECK_ERROR(hipSignalExternalSemaphoresAsync(
        (hipExternalSemaphore_t *)&hSemaphore, &SemSignalParams,
        1 /* numExtSems */, Stream));
